...
export default function Home() {
  ...
  function assignDrSlot(date: string): 'DR.1'|'DR.2'|'DR.3'|null {
    const existing = queue.filter(e => e.header.date===date && e.id!==editingId)
    if (existing.length===0) return 'DR.1'
    if (existing.length===1) return 'DR.2'
    if (existing.length===2) return 'DR.3'
    return null
  }

  function getDayNumber(date: string): number {
    const allDates = [...new Set(queue.map(e=>e.header.date))]
  ...
  function handleConfirm() {
    ...
    if (editingId) {
      ...
      const entry: QueueEntry = { id:`entry-${Date.now()}`, dayNumber:getDayNumber(date), drSlot:slot, header:current.header, items:current.items }
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{background:'#f5f4f0'}}>
      <header className="sticky top-0 z-30 flex items-center gap-3 px-6 py-3 shadow-md" style={{background:'#1a1a2e'}}>
        ...
        <div className="ml-auto">
          {queue.length>0 && <span className="bg-white/10 text-white text-xs px-3 py-1 rounded-full">Queue: {queue.length} รายการ</span>}
        </div>
      </header>
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-0 lg:gap-6 p-4 lg:p-6 max-w-7xl mx-auto w-full">
        <div className="flex flex-col gap-4">
          <div className="bg-white rounded-2xl shadow-sm p-4 space-y-3">
            <div className="flex items-center justify-between">
              <h2 className="font-semibold text-sm text-gray-700">{isEditing ? '✏️ แก้ไขรายการ' : '📋 งานปัจจุบัน'}</h2>
              {isEditing && <button onClick={() => {setEditingId(null);setCurrent(null)}} className="text-xs text-gray-400 hover:text-gray-600">ยกเลิกแก้ไข</button>}
            </div>
            {!isEditing && (
              <>
                <UploadZone onFileSelect={handleFileSelect} preview={preview} disabled={analyzing} />
                {file && !current && (
                  <button onClick={handleAnalyze} disabled={analyzing}
                    className={`w-full py-2.5 px-4 rounded-xl font-semibold text-sm transition-all ${analyzing?'bg-blue-300 text-white cursor-not-allowed':'bg-blue-600 hover:bg-blue-700 text-white shadow hover:shadow-md active:scale-95'}`}>
                    {analyzing ? (
            ...
            {error && <div className="bg-red-50 border border-red-200 rounded-lg px-3 py-2 text-sm text-red-600">⚠️ {error}</div>}
          </div>
          {current && (
            <>
        ...
        <div className="flex flex-col gap-4">
          <div className="bg-white rounded-2xl shadow-sm p-4 flex-1">
            ...
            <QueueList queue={queue} onEdit={handleEdit} onDelete={id=>setQueue(prev=>prev.filter(e=>e.id!==id))} />
          </div>
